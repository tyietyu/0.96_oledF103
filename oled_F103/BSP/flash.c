...
void Flash_PageErase(uint32_t PageAddress)
{
  ...
  * 说    明： 
  */
void iap_write_flash(uint32_t WriteAddr,uint16_t *pBuffer,uint16_t NumToWrite)	
{
	...
	while(1) 
	{	
		iap_read_flash(secpos*STM_SECTOR_SIZE+STM32_FLASH_BASE,STMFLASH_BUF,STM_SECTOR_SIZE/2);//读出整个扇区的内容
  ...
  * 说    明： 
  */
void iap_read_flash(uint32_t ReadAddr,uint16_t *pBuffer,uint16_t NumToRead)   	
{
	uint16_t i;
