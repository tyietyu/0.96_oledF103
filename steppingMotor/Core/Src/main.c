...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      float speed = 0.0f;
      if (sscanf((char*)uart_rx_data.buffer, "s%f", &speed) == 1)
      {
        hal_steppingMotor_set_target_speed(speed);
      }
      // Clear buffer and reset flag
      memset(uart_rx_data.buffer, 0, uart_rx_data.recv_length);
      uart_rx_data.recv_length = 0;
      uart_rx_data.flag = 0;
    }
    /* USER CODE END WHILE */
